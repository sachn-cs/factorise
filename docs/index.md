# Algorithm Documentation

This directory contains curated narrative documentation for the factorisation algorithms used in the `factorise` package.

## Available Documentation

- [Miller-Rabin Primality Test](miller_rabin.md)
- [Pollard's Rho](pollards_rho.md)
- [Pollard's Rho (Brent Variant)](pollards_rho_brent.md)
- [Trial Division](trial_division.md)
- [Pollard p-1](pollard_pm1.md)
- [Elliptic Curve Method (ECM)](ecm.md)
- [Quadratic Sieve](quadratic_sieve.md)
- [Self-Initializing Quadratic Sieve (SIQS)](siqs.md)
- [General Number Field Sieve (GNFS)](gnfs.md)
- [References](references.md)

## Note

These are narrative/explanatory docs, not generated API reference. For the full API, see the docstrings in the source code and `README.md`.
